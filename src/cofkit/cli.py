...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="cofkit", description="CLI for COF generation, screening, and validation.")
    subparsers = parser.add_subparsers(dest="command", required=True)

    _add_single_pair_parser(subparsers)
    _add_batch_binary_bridge_parser(subparsers)
    _add_batch_imine_parser(subparsers)
    _add_batch_all_binary_bridges_parser(subparsers)
    _add_classify_output_parser(subparsers)
    _add_build_default_library_parser(subparsers)
    _add_list_templates_parser(subparsers)
    return parser
...
def _configure_generator(args: argparse.Namespace, *, template_id: str | None = None) -> BatchStructureGenerator:
    allowed_reactions = (template_id,) if template_id is not None else ((args.template_id,) if hasattr(args, "template_id") else ("imine_bridge",))
...
def _add_batch_imine_parser(subparsers) -> None:
    parser = subparsers.add_parser(
        "batch-imine",
        help="Run the imine batch workflow.",
        description="Run batch imine COF generation over monomer libraries.",
    )
    _add_common_batch_generation_arguments(parser)
    parser.set_defaults(
        func=_run_batch_imine,
        input_dir=str(Path(__file__).resolve().parents[2] / "examples" / "batch_test_monomers"),
        output_dir=str(Path(__file__).resolve().parents[2] / "out" / "batch_imine_generation"),
    )


def _run_batch_imine(args: argparse.Namespace) -> None:
    generator = _configure_generator(args, template_id="imine_bridge")
    ...
    _print_batch_summary(summary, template_id="imine_bridge")
...
def _add_build_default_library_parser(subparsers) -> None:
    parser = subparsers.add_parser(
    ...
    parser.set_defaults(func=_run_build_default_library)


def _run_build_default_library(args: argparse.Namespace) -> None:
    input_dir = Path(args.input_dir)
    output_dir = Path(args.output_dir)
