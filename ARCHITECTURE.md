...
- `cofkit.decompose_bond_orders`
  - shared pre-detection normalization for explicit Kekule CIFs: short H-explicit CH=N links constrain a valence-preserving perfect matching of neutral C/N conjugated components; the remaining matching favors the original bond assignment and then the shorter contacts, so equal-preservation ties follow the geometry instead of the atom row order. Both event and legacy graph construction use the normalized orders while retaining every distinct periodic candidate edge and recording changes in metadata. Per-pair periodic edge multiplicities are passed into normalization to validate per-atom periodic valences before mutation. A failing proposal is retried with parallel-image bond orders fixed; rejection leaves only that component unchanged. This does not expand the cell or infer new connectivity.
- `cofkit.engine`
...
2. `cofkit.cli_build`, `cofkit.cli_analyze`, and `cofkit.cli_calculate` own the grouped `cofkit build ...`, `cofkit analyze ...`, and `cofkit calculate ...` surfaces.
