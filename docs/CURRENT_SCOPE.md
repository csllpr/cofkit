...
- working UFF- and DREIDING-backed LAMMPS parameterization paths using Open Babel UFF atom typing, the bundled pinned Open Babel `UFF.prm` reference for `UFF`, pinned `lammps-interface` DREIDING tables/formulas for `DREIDING`, `pymatgen` LAMMPS-data generation for bond, angle, dihedral, improper, and van der Waals terms, and default EQeq charge staging for charged `atom_style full` exports; `UFF` remains the CLI default for compatibility, but `DREIDING` is now the preferred practical backend while `UFF` should be treated as experimental support, and local `spring/self` restraints now contribute to the minimized objective through `fix_modify energy yes`
- documented temporary forcefield-parameter review items: the current `DREIDING` data are pinned to `lammps-interface` commit `255f027cb76142d39c050a6810404debc6a06562`, some heavier-atom entries there are explicitly heuristic, and the current generated `UFF` / `DREIDING` gRASPA framework rows may differ slightly from older rounded template/example files
