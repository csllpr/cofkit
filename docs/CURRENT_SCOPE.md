...
- an initial `cofkit calculate graspa-widom` wrapper for staged `EQeq -> gRASPA/RASPA2` Widom insertion on one CIF, including env-based executable discovery through `COFKIT_EQEQ_PATH`, `COFKIT_GRASPA_PATH`, and `COFKIT_RASPA2_PATH` with automatic nearest-`.env` loading in the CLI, backend selection through `--backend {graspa,raspa2}`, packaged adsorbate-template materialization plus generated framework `force_field_mixing_rules.def` assets for selectable `DREIDING` / `UFF` forcefield families, automatic `UnitCells` sizing from CIF cell lengths plus cutoff settings, parsed `results.csv` generation, and `graspa_widom_report.json` output
...
- broader gRASPA/RASPA2 workflow coverage beyond the current bundled adsorbate/template family, selectable `DREIDING` / `UFF` framework assets, Widom/isotherm/mixture wrappers, and the present Henry-coefficient / Widom-energy / absolute-loading / heat-of-adsorption parsers
