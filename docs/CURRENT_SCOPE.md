...
- an initial `cofkit analyze decompose` command for recovering a COFid from one supported atomistic CIF, currently limited to explicit-bond imine CIFs with caller-supplied topology; the decomposition approach is adapted from the deCOFpose project at `https://github.com/r-fedorov/deCOFpose`
...
- CIF decomposition beyond explicit-bond imine structures, including automatic topology inference and robust support for CIFs without usable bond loops
