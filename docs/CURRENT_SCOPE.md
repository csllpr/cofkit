...
- an initial `cofkit calculate graspa-isotherm` wrapper for staged `EQeq -> gRASPA` single-component adsorption isotherms on one CIF, including one packaged adsorbate definition per run, one or more explicit pressure points, shared `UnitCells` sizing from the charged CIF plus cutoff settings, per-pressure staged run directories, aggregated `isotherm/results.csv` generation, and `graspa_isotherm_report.json` output
