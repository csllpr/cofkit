...
- an initial `cofkit calculate graspa-widom` wrapper for staged `EQeq -> gRASPA` Widom insertion on one CIF, including env-based executable discovery through `COFKIT_EQEQ_PATH` and `COFKIT_GRASPA_PATH` with automatic nearest-`.env` loading in the CLI, packaged Widom-template asset materialization, automatic `UnitCells` sizing from CIF cell lengths plus cutoff settings, parsed `results.csv` generation, and `graspa_widom_report.json` output
- an initial `cofkit calculate graspa-isotherm` wrapper for staged `EQeq -> gRASPA` single-component adsorption isotherms on one CIF, including one packaged adsorbate definition per run, one or more explicit pressure points, shared `UnitCells` sizing from the charged CIF plus cutoff settings, per-pressure staged run directories, aggregated `isotherm/results.csv` generation, and `graspa_isotherm_report.json` output, with executable discovery still driven by `COFKIT_EQEQ_PATH` and `COFKIT_GRASPA_PATH` and the same CLI `.env` autoload behavior
- an initial `cofkit calculate graspa-mixture` wrapper for staged `EQeq -> gRASPA` multi-component adsorption/selectivity on one CIF, including repeated `NAME:FRACTION` component definitions, one or more explicit pressure points, shared `UnitCells` sizing from the charged CIF plus cutoff settings, per-pressure staged run directories, aggregated `mixture/component_results.csv` and `mixture/selectivity_results.csv` generation, and `graspa_mixture_report.json` output
...
- broader gRASPA workflow coverage beyond the current bundled Widom-template family, selectable packaged probe set, single-component isotherm wrapper, and the present Henry-coefficient / Widom-energy / absolute-loading / heat-of-adsorption parsers
