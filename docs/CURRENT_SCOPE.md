...
- an initial `cofkit calculate hybrid-mdmc` wrapper for cyclic LAMMPS MD plus gRASPA/RASPA2 GCMC on one explicit-bond framework CIF, including per-cycle LAMMPS MD handoff CIF export, per-cycle pure-component or mixture GCMC at one pressure, default framework snapshot exchange into the next cycle, opt-in `guest_restart` exchange that parses final GCMC `Movies/System_0/result_*.data` snapshots and injects massive guest atoms into the following LAMMPS MD segment, synchronized packaged or guest-bundle guest force-field rows for gRASPA/RASPA2 and LAMMPS, binary guest mixtures, shared EQeq settings for the LAMMPS and GCMC charge stages, and a top-level `hybrid_mdmc_report.json`
...
- automatic arbitrary-SMILES guest parameterization, massless/dummy-site guest MD treatment for pseudo-sites such as COM or off-center charge sites, post-MD guest-coordinate conversion into gRASPA/RASPA2 `RestartInitial` input, and full bidirectional dynamic-GCMC-style MD/MC cycling beyond the current framework and GCMC-to-MD guest-restart `hybrid-mdmc` exchange modes
