...
- an initial `cofkit analyze decompose` command for recovering a COFid from one supported atomistic CIF, covering the buildable explicit-bond binary-bridge linkages (`imine`, `hydrazone`, `azine`, `boest`, `bken`, and `vinylene`) with caller-supplied topology; the decomposition approach is adapted from the deCOFpose project at `https://github.com/r-fedorov/deCOFpose`
...
- CIF decomposition beyond the current buildable explicit-bond binary-bridge structures, including automatic topology inference and robust support for CIFs without usable bond loops
