...
Two decomposition engines share those graph and validation primitives. `legacy` remains the default and preserves the established independent per-family control flow. The opt-in `event` engine detects immutable local linkage events, applies only local overlap priorities, enumerates bounded per-site reconstruction hypotheses, and makes the final assignment after precursor, endpoint, framework-accounting, periodic, topology, and forward-build validation. Event mode also treats azine and boronate ester as indivisible paired events, branches tied vinylene orientations and alternating triazine nitrile matchings, preserves triazine nitrogen atoms during reverse reconstruction, and applies the triazine motif-versus-linkage decision against every complete competing family. It is experimental until an independent labelled benchmark supports replacing the default; architecture and benchmark requirements are documented in [event-decomposition.md](event-decomposition.md).
...
The vinylene marker first collects formal C=C bonds, preserves the existing same-instance rejection when COFKit labels are available, and then rejects bonds belonging to local five- or six-membered rings. Remaining bonds require carbon anchors on both endpoints and a unique activated side whose anchor is electron-withdrawing through multiple nitrogen neighbors or a multiple bond to N/O/S. Recognized boronate-ester B-O bonds and complete boroxine rings are retained in diagnostic metadata but no longer suppress a structurally separate valid vinylene bond. This prevents Kekule-encoded aromatic bonds and ordinary cyclic or unactivated alkenes from being cut while avoiding a structure-wide chemistry veto.

Triazine ring decomposition independently evaluates the binary imine and vinylene hypotheses. A triazine assignment is withheld as ambiguous only if a competing strategy recovers valid precursor motifs and a rank-2 or rank-3 periodic backbone. The alternating formal C=N bonds within a Kekule triazine ring are therefore not, by themselves, evidence of an imine linkage.
