...
3. [`03_zeopp_pore_analysis.ipynb`](03_zeopp_pore_analysis.ipynb) discovers Notebook 1's CIF and runs cofkit's Zeo++ wrapper with a configured Windows-runnable executable.

The executable cells use `%%script powershell -NoProfile` and `conda run -n cofkit`. Corresponding Python API examples remain fully commented. Zeo++ 0.3 documents Windows compilation through Cygwin and GNU Make; in restricted environments, an instructor-provided executable is the practical option.
