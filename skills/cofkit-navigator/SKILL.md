...
- `cofkit analyze decompose`: recover a COFid from one supported atomistic CIF. Current support is explicit-bond imine CIFs with caller-supplied topology.
...
- `cofkit analyze decompose` currently requires an atomistic CIF with explicit `_geom_bond_*` connectivity, supports only imine decomposition, and requires `--topology` because topology inference is not implemented. Its decomposition logic was adapted from the deCOFpose project at `https://github.com/r-fedorov/deCOFpose`.
...
- CIF decomposition: explicit-bond imine CIF path and topology token for `--topology`; add `--json` only when diagnostics are useful
