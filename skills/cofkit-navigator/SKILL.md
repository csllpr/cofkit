...
- `cofkit analyze decompose`: recover a COFid from one supported atomistic CIF. Current support covers explicit-bond CIFs for the buildable binary-bridge linkages with caller-supplied topology.
...
- CIF decomposition: explicit-bond CIF path, topology token for `--topology`, and `--linkage` when the structure is not imine; add `--json` only when diagnostics are useful
